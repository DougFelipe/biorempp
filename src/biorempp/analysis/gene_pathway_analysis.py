...
def process_ko_data(merged_df) -> pd.DataFrame:
    """
    Legacy function for processing KO data.

    This function is deprecated. Use
    GenePathwayAnalyzer.analyze_ko_counts_per_sample() instead.
    ...
    -------
    pd.DataFrame
        A DataFrame with samples and their respective unique KO counts,
    ...
    """
    logger.warning(
        "process_ko_data is deprecated. Use "
        "GenePathwayAnalyzer.analyze_ko_counts_per_sample() instead"
    )
    ...
    if isinstance(merged_df, list):
        ...
        raise ValueError(
            "The merged_df argument must be a pandas DataFrame "
            "or a list of dictionaries."
        )

    # Use new analyzer
    analyzer = GenePathwayAnalyzer()
    return analyzer.analyze_ko_counts_per_sample(merged_df)
