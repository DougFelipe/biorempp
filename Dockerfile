...
# and `pip install biorempp` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "biorempp", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n biorempp -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "biorempp", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "biorempp", "YOUR_CONSOLE_SCRIPT"]
